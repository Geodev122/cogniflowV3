...
-- This archived demo migration previously inserted demo profiles, sessions and onboarding rows
-- which assumed specific `auth.users` and `public.profiles` UUIDs existed. Running those
-- inserts against a database without the matching `auth.users` will create invalid rows or
-- violate NOT NULL/foreign-key constraints (and may create `profiles` with NULL user_id).
--
-- To avoid accidental data corruption, the interactive/demo inserts in this archived file
-- have been disabled. The original SQL is preserved below inside an IF FALSE guard so the
-- history remains visible but the statements won't execute during migration runs.

DO $$
BEGIN
  -- Disabled: archived demo seeds (see repository README for targeted seeding instructions)
  IF FALSE THEN
